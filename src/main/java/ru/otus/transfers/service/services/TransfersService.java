...
public class TransfersService {
    ...
    protected void executeTransfer(String clientId, ExecuteTransferDtoRq executeTransferDtoRq) {
        ...
        Account sourceAccount = accountService.findByAccountNumberAndClientId(sourceAccountNumber, clientId)
                .orElseThrow(() -> new BusinessLogicException("Счет отправителя не найден", "SOURCE_ACCOUNT_NOT_FOUND"));

        if (sourceAccount.isBlocked()) {
            throw new BusinessLogicException("Счет отправителя заблокирован", "SOURCE_ACCOUNT_IS_BLOCKED");
        }

        if (!accountService.existsByAccountNumber(targetAccountNumber)) {
            throw new BusinessLogicException("Счет получателя не найден", "TARGET_ACCOUNT_NOT_FOUND");
        }
        if (accountService.isBlocked(targetAccountNumber)) {
            throw new BusinessLogicException("Счет получателя заблокирован", "TARGET_ACCOUNT_IS_BLOCKED");
        }

        if (sourceAccount.getBalance() - transferAmount < 0) {
            throw new BusinessLogicException("Недостаточно средств", "INSUFFICIENT_BALANCE");
        }
